    ...
    class DataPreparation
    {
        ...
        public static void MakeSearchableData(string whole_path, string interm_path, string final_path, int start, int total)
        {
            ...
            for (; i < fin; i++)
            {
                ...
                var tags = whole_db.Table<QuestionTags>().Where(f => f.QuestionId == q.Id).Select(f => new { f.TagId });
                var tag = whole_db.Table<Tag>().IntersectListInt(f => f.Id, tags.Select(f => f.TagId).ToList()).Select(f => new { f.Name }).Take(2).Select(f => f.Name).Aggregate((a, b) => a + " " + b);
                current++;
                if (current > total)
                {
                    break;
        ...
        public static void MakeFragments(string interm_path, string final_path)
        {
            ...
            for (int i = start; i <= fin; i += step)
            {
                var res = interm_db.Table<WholePost>()
                                   .BetweenInt(f => f.Id, i, i + step, BetweenBoundaries.FromInclusiveToExclusive)
